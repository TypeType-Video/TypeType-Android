...
| --- | --- | --- |
| TypeType-Android | `dev` | `ed7d8338dffb643f437508dac3ef5d78ca3683c7` |
...
| Background, notification, lock screen, and headset controls | One MediaSession | Implemented | A secondary controller, the foreground playback notification, PiP actions, and real system media-key pause and play all control the same service-owned player. The media-key path passes on API 23 without Google services, API 29, API 34, and API 37 |
...
| Playback restoration after process death | Stable video identity, `/progress` | Implemented | Room close/reopen restores exact position, queue, repeat mode, and account scope without persisting signed media URLs. The current 15-test persistence suite also covers import jobs, completed download artifacts, feed caches, playlist summaries, cookies, and every affected Room migration on API 23, 29, 34, and 37 |
