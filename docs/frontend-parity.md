...
| Keep the display awake during video playback | Device window state | Implemented | The window flag follows the single MediaSession playback state in portrait, fullscreen, and Shorts. Its lifecycle passes on API 23, 29, and 37; on API 29, a real beta VOD stayed awake in portrait and fullscreen with a five-second system timeout, while pausing removed the flag and allowed the display to sleep |
