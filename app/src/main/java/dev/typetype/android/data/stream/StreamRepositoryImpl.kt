...
class StreamRepositoryImpl @Inject constructor(
    ...
    private fun StreamResponse.toDomain(
        ...
        return Stream(
            ...
            audioStreams = audioStreams
                ...
                .filter { it.deliveryMethod != SABR_DELIVERY_METHOD }
                .map { it.toDomainAudioSource(baseUrl) },
            subtitles = subtitles.map { it.toDomainSubtitleSource(baseUrl) },
    ...
    private fun SubtitleItem.toDomainSubtitleSource(baseUrl: String): StreamSubtitleSource = StreamSubtitleSource(
        url = proxiedVttUrl(baseUrl, url) ?: url,
        mimeType = "text/vtt",
    ...
    private fun proxiedVttUrl(baseUrl: String, rawUrl: String): String? {
        val normalizedBaseUrl = baseUrl.trimEnd('/')
        val vttUrl = rawUrl.toHttpUrlOrNull()
            ...
            ?.toString()
            ?: return null
        return "$normalizedBaseUrl/proxy?url=${URLEncoder.encode(vttUrl, StandardCharsets.UTF_8)}"
    }
