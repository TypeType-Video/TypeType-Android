...
@Singleton
class LibraryMutationScheduler @Inject constructor(
    @ApplicationContext context: Context,
) {
    private val workManager = WorkManager.getInstance(context)

    fun enqueue(scope: AccountScope, baseUrl: String, generation: Long) {
        val request = OneTimeWorkRequestBuilder<LibraryMutationWorker>()
            .setInputData(LibraryMutationWorkContract.input(scope.serverId, scope.accountId, baseUrl, generation))
            .setConstraints(Constraints.Builder().setRequiredNetworkType(NetworkType.CONNECTED).build())
            .setBackoffCriteria(BackoffPolicy.EXPONENTIAL, MIN_BACKOFF_SECONDS, TimeUnit.SECONDS)
            .build()
        workManager.enqueueUniqueWork(workName(scope, generation), ExistingWorkPolicy.REPLACE, request)
    }
    ...
    private fun workName(scope: AccountScope, generation: Long): String {
        ...
        return "typetype-library-$digest"
    }
